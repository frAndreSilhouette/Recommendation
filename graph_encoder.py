...
class GraphConvolutionalEncoder(nn.Module):
    ...
    """
    def __init__(self, adj_matrix, embed_dim=64, num_layers=2, device='cuda'):
        super().__init__()
        self.device = device
        self.adj = adj_matrix.to(device)  # 将稀疏矩阵放到指定 device 上

        # 自动获取节点数量
        self.num_items = self.adj.shape[0]
    ...
    def forward(self):
        ...
        for layer in range(self.num_layers):
            ...
            e = e + torch.sparse.mm(self.adj, e)
...
if __name__ == "__main__":
    ...
    # 1. 读取稀疏邻接矩阵，并放到 GPU
    adj = torch.load('item_graph.pt').to(device)  # 强制放到 cuda

    # 2. 构建 GCN encoder
    gcn_encoder = GraphConvolutionalEncoder(adj_matrix=adj, embed_dim=64, num_layers=2, device=device)
    ...
    # 4. 保存到 cpu
    torch.save(item_embeddings.cpu(), "item_graph_embeddings.pt")
